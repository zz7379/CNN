...
import numpy
CLASSNUM = 15#10
IMGSIZE = 22*(1000 - 31 * 3)#2400
CHANNEL = 18
def extract_images(data):
  """Extract the images into a 4D uint8 numpy array [index, y, x, depth]."""
  if CHANNEL != 1:
    data = data.reshape(data.shape[0], -1, CHANNEL)
  else:
    data = data.reshape(data.shape[0], data.shape[1])
  return data
def dense_to_one_hot(labels_dense, num_classes=CLASSNUM):
...
class DataSet(object):
  ...
  def next_batch(self, batch_size, fake_data=False):
    ...
    if fake_data:
      fake_image = [1.0 for _ in xrange(IMGSIZE)]
      fake_label = 0
...
def read_data_sets(images, labels, test_images, test_labels,fake_data=False, one_hot=True):
  ...
  TRAIN_RATE = 0.75
  TRAIN_SIZE = int(images.shape[0] * 0.75)
